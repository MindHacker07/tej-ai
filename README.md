...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   01 ─ USER     │    │   02 ─ LLM      │    │   03 ─ EXECUTE  │    │   04 ─ FEEDBACK │
│     PROMPT      │───>│   PROCESSING    │───>│     TESTS       │───>│      LOOP       │
│                 │    │                 │    │                 │    │                 │
│ • Natural lang  │    │ • Claude / GPT  │    │ • Run selected  │    │ • Parse output  │
│   query input   │    │   / Groq /      │    │   security      │    │ • Extract       │
│ • Instructions  │    │   Ollama        │    │   tools         │    │   findings      │
│   & context     │    │ • Routes via    │    │ • Security      │    │ • Suggest next  │
│ • Target info   │    │   MCP servers   │    │   analysis      │    │   iteration     │
│                 │    │ • Intent parse  │    │ • Capture       │    │ • Update        │
│                 │    │ • Tool select   │    │   results       │    │   session       │
└─────────────────┘    └─────────────────┘    └─────────────────┘    └────────┬────────┘
                                                                              │
                       ┌──────────────────────────────────────────────────────┘
                       │  Next Iteration
...
```
User Prompt ──► LLM Processing ──► Execute Tests ──► Feedback Loop
    │            (Routes via MCP)   (Security         (Next Iteration)
    │                                Analysis)              │
    └──────────────────────────────────────────────────────┘
```
